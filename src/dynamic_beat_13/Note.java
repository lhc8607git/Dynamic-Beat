...
public class Note  extends Thread{    //각각의 노트 또한  하나의 부분적인 기능으로서 떨어지는 어떤한 역할을 수행하기때문에 Thread로 만들어 주어야 한다
	...
	private int x , y = 580 - 1000 / Main.SLEEP_TIME * Main.NOTE_SPEED;  //현재 노트가 어느 위치에 있는지 보여주는거    //정확하게 노트가 생성되고 1초뒤에 판정라인에 다다르게 되는 거다
	private String noteType; 

	public Note(int x,  String noteType) {   //생성자 만듬 //현재 노트가 만들어질 위치를 만들어주고 
		this.x = x;
	...
	public void screenDraw(Graphics2D g) {     //★하나의 노트의 이미지★를 ★그릴 수 있도록★  ★screenDraw()★를 만들어 줄 필요가 있다
		...
		else if(noteType.equals("long"))     //긴 노트라면
		{
